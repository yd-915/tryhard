...
async function registerUser(userData) {
    ...
    if (checkUser) errors.push('This email address is already taken!; ');
    if (name.length < 3 || name.length > 50) errors.push('Name should be at least 3 characters and max 50 characters; ')
    if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email) == false) errors.push("Please give a valid email address; " );
